...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ ok: true, job: result.job, metadata: result.job.metadata, safetyCaps: result.caps, persistence: result.job.persistence, persistenceWarning: result.job.persistenceWarning } satisfies ChannelFetchStartResponse);
  } catch (error) {
